    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("box", scope))
...
def solve_planes(planes_problem, algo, allsolns=True,
    ...
    csp = planeCSP(planes_problem) #set to to your CSP
    ...
    for solution in solutions:
        next_solution = {plane: [] for plane in planes_problem.planes}
        for var, val in solution:
            plane_name, plane_sequence = var.name().split("_")
            if type(val) == int:
                continue
            else:
                next_solution[plane_name].append(val)
        final_solution = []
        for plane in next_solution:
            final_solution.append([plane] + next_solution[plane])
        converted_solutions.append(final_solution)
...
def planeCSP(p0):
    ...
    # Initialize Variables that already applies can_fly & start_fly constraint
    none_base = max([len(p0.can_fly(plane)) for plane in p0.planes])
    none_pairs_dict = {}
    none_vals_dict = {}

    for j, plane in enumerate(p0.planes):
        none_pairs_dict[plane] = []
        none_vals_dict[plane] = []
        for idx, flight in enumerate(p0.can_fly(plane)):
            none_idx = none_base * j + idx
            none_vals_dict[plane].append(none_idx)
            if idx == 0:
                nxtvar = Variable(plane + f"_{idx}", [none_idx] + [flight for flight in p0.can_start(plane)])
            else:
                nxtvar = Variable(plane + f"_{idx}", [none_idx] + [flight for flight in p0.can_fly(plane)])
                none_pairs_dict[plane].append([none_idx - 1, none_idx])
                if idx == 1:
                    none_pairs_dict[plane].extend([[flight, none_idx] for flight in p0.can_start(plane)])
                else:
                    none_pairs_dict[plane].extend([[flight, none_idx] for flight in p0.can_fly(plane)])

            var_dict[(plane, idx)] = nxtvar
            var_list.append(nxtvar)

    # can_follow constraint
    new_can_follow = [list(x) for x in p0.can_follow]
    for plane in p0.planes:
        for idx in range(len(p0.can_fly(plane)) - 1):
            constraint_list.append(
                TableConstraint(f"{plane}_{idx}_{idx + 1}", [var_dict[(plane, idx)], var_dict[(plane, idx + 1)]],
                                new_can_follow + none_pairs_dict[plane]))

    # all_flights constraints (number of flights scheduled)
    constraint_list.append(
        NValuesConstraint("num_flights", var_list, p0.flights, len(p0.flights), len(p0.flights)))

    # all_flights constraints (all flights are scheduled)
    constraint_list.append(
        AllDiffConstraint("diff_flights", var_list))

    # maintenance constraint
    for plane in p0.planes:
        combo_length = min(len(p0.can_fly(plane)), p0.min_maintenance_frequency)
        constraint_list.extend(
            [NValuesConstraint(plane + f"{j}",
                               [var_dict[(plane, idx + j)] for idx in range(combo_length)],
                               p0.maintenance_flights + none_vals_dict[plane],
                               1,
                               combo_length) for j in range(len(p0.can_fly(plane)) - p0.min_maintenance_frequency + 1)])

    return CSP("Plane", var_list, constraint_list)
