...
class NValuesConstraint(Constraint):
    ...
    def __init__(self, name, scope, required_values, lower_bound, upper_bound):
        ...
        self._required = set(required_values)
        ...
        for v in self.scope():
            if v.isAssigned():
                ...
                if v.getValue() in self._required:
                    count += 1
        return self._lb - len(self.scope()) + num_assigned <= count <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def nvalues_check(l):
            count = 0
            for _var, _val in l:
                if _val in self._required:
                    count += 1
            return self._lb - len(varsToAssign) <= count <= self._ub

        return findvals(varsToAssign, [(var, val)], nvalues_check, nvalues_check)
