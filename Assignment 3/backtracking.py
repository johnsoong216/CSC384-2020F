        ...
        for cnstr in csp.constraintsOf(nxtvar):
            if cnstr.numUnassigned() == 0:
                if not cnstr.check():
                    ...
                    # print(cnstr)
                    # print([d.curDomain() for d in cnstr.scope()])
                    break
        if constraintsOK:
    ...
    for val in var.curDomain():
        ...
        if noDWO:
            ...
            if len(solns) > 0 and not allSolutions:
                ...
                break  # don't bother with other values of var as we found a soln.
        Variable.restoreValues(var, val)
    var.unAssign()
            ...
            for val in var.curDomain():
                if not constraint.hasSupport(var, val):
                    ...
                    if var.curDomainSize() == 0:
                        ...
                        # print(constraint.scope())
                        # print("Domain 0: ", constraint.scope()[0].domain())
                        # print("Current Domain 0: ", constraint.scope()[0].curDomain())
                        # print("Domain 1: ", constraint.scope()[1].domain())
                        # print("Current Domain 1: ", constraint.scope()[1].curDomain())
                        # print("Val: ", val)
                        return "DWO"
