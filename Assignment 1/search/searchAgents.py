...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        corners_reached = [0] * len(self.corners)
        try:
            corners_reached[self.corners.index(self.startingPosition)] = 1
        except ValueError:
            pass
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return sum(state[1]) == len(state[1])
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                try:
                    cur_corners[self.corners.index(next_state)] = 1
                except ValueError:
                    pass
                successors.append(
                    ((next_state, tuple(cur_corners)), action, 1)
                )
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    visit_corners = [corners[idx] for idx in range(len(corners)) if state[1][idx] != 1]
    cur_position = state[0]
    total_distance = 0
    while visit_corners:
        min_distance, min_corner, min_idx = min(
            [(norm_distance(cur_position, corner, 2), corner, idx) for idx, corner in enumerate(visit_corners)], key=lambda x:x[0])
        total_distance += min_distance
        cur_position = min_corner
        visit_corners.pop(min_idx)
...
def foodHeuristic(state, problem):
    ...
    # Construct an Adjacency matrix to facilitate graph calculation
    if "foodGridMatrix" not in problem.heuristicInfo:
        ...
        for i in range(len(foodGridMatrix)):
            for j in range(len(foodGridMatrix)):
                foodGridMatrix[i][j] = norm_distance(foodGridList[i], foodGridList[j], 1)
                foodGridMatrix[j][i] = foodGridMatrix[i][j]

        # Store it as part of the heuristic info
        problem.heuristicInfo["foodGridDict"] = foodGridDict
        problem.heuristicInfo["foodGridMatrix"] = foodGridMatrix

    #     problem.heuristicInfo["Count"] = 0
    #
    # problem.heuristicInfo["Count"] += 1
    # if problem.heuristicInfo["Count"] % 1000 == 0:
    ...
    fGList = foodGrid.asList() + [position]
    fGDict = problem.heuristicInfo["foodGridDict"]
    fGMatrix = problem.heuristicInfo["foodGridMatrix"]
        ...
        for j in fGList:
            if i >= j:
                ...
                if j != position and i != position:
                    ...
                    food_graph.append([i,j, norm_distance(i,j, 1)])
...
# Extra Helper
def norm_distance(p1, p2, deg):
    return (abs(p1[0] - p2[0])**deg + abs(p1[1] - p2[1])**deg)**(1/deg)
